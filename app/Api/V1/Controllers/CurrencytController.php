...
class CurrencytController extends BaseController
{
    ...
    public function getInfo(Request $request){
      ...
      //                 ->groupby('users.id')
      //                 ->orderby('deposit_count', 'DESC');
      //   $insts = $query->where('roles.name', '='  , config('constants.INSTITUTION_USER'))->get();
      //   if ($insts != null && count($insts) > 0) {
      //     $inst = $insts[0];
      //     $inst->image_url = $inst->image_url?url('/').$inst->image_url:"";
      ...
      //   $schools = $query->where('roles.name', '='  , config('constants.SCHOOL_USER'))->get();
      //   if ($schools != null && count($schools) > 0) {
      //     $school = $schools[0];
      //     $school->image_url = $school->image_url?url('/').$school->image_url:"";
      //     $res['school'][] = $school;
      //   } else {
      //     $res['school'][] = null;
      //   }
